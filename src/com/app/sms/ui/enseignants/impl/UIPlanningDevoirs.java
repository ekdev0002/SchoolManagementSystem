...
public class UIPlanningDevoirs extends AbstractUIOperation implements IUIPlanningDevoirs {
	...
	public UIPlanningDevoirs() {
		...
		panel_22.setBorder(new TitledBorder(null, "Modifier Devoirs", TitledBorder.LEADING, TitledBorder.TOP, null, null));
        ...
        JLabel lblId1 = new JLabel("Statut");
        panel_81.add(lblId1);
        
        statut = new JComboBox<>();
        panel_22.add(statut);
        statut.addItem("pas encore effectué");
        statut.addItem("effectué");
		...
		listeDevoirs.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			@Override
	...
	public void updateUIForm(String[] data) {
		try {
			...
			else
			{
				...
				this.setStatut(data[3]);
				this.setDateHeure(data[4]);
				this.setDuree(data[5]);
	...
	private void setStatut(String string) {
		...
		for (int i=0;i<statut.getItemCount();i++)
		{ 	
			
			if((statut.getItemAt(i).toString()).equals(string))
			{
				statut.setSelectedItem(statut.getItemAt(i));
			}
		}
		}
...
//	}
	...
	public String getStatut() {
		// TODO Auto-generated method stub
		return ajouterSelected() ? "pas encore effectue" : statut.getSelectedItem().toString();
	}
}
