...
public class UIConsulterEnseignant extends AbstractUIOperation implements IUConsulterEnseignant {
	...
	public UIConsulterEnseignant() {
		...
		listeEnseignantPanel.setBackground(Color.WHITE);
				...
				try {
					...
					for ( int i = selectionModel.getMinSelectionIndex() ; i <= selectionModel.getMaxSelectionIndex() ; i ++ ) {
						if (selectionModel.isSelectedIndex(i)) {
							...
							List<Candidature> candidatures = enseignant.getCandidatures();
		...
		visualiserEnseignantPanel.setBackground(Color.WHITE);
		...
		JLabel listeClassesLabel = new JLabel("  Classes : ");
		...
		scrollPaneListeClasses.setMaximumSize(new Dimension(70, 15));
		...
		updateClassePanel.setLayout(new GridLayout(2, 0, 0, 0));
	...
	private void updateUIEnseignant(Enseignant enseignant) {
		try {
			...
			this.picturePath = enseignant.getPicturePath();
			if (enseignant.getPicturePath() == null) setDefaultPicture();
			else {
				ImageIcon imageIcon = Utilitaire.getImageIcon (this, picturePath);
				setPicture(imageIcon);
			}
			List<Classe> classes = enseignant.getClasses();
			List<Classe> touteLesclasses = Classe.list();
